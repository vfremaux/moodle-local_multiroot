...
if (empty($CFG->multiroot)) {
    // let the standard  code pass.
    return;
}

// Add multiroot filtering.
require_once($CFG->dirroot.'/filter/multiroot/filter.php');
$filter = new filter_multiroot();
...
if (!empty($notification->contexturl)) {
    $redirecturl = new moodle_url($filter->filter($notification->contexturl));
} else {
